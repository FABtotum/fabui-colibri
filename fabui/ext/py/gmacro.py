...
if force == 0 and os.path.isfile(FabtotumConfig.LOCK_FILE):
    print "printer busy"
    sys.exit()
...
json_f = open(FabtotumConfig.HW_DEFAULT_SETTINGS)
units = json.load(json_f)
#process params

if 'settings_type' in units and units['settings_type'] == 'custom':
    json_f = open(FabtotumConfig.HW_CUSTOM_SETTINGS)
    units = json.load(json_f)
...
trace_file=FabtotumConfig.TRACE
response_file=FabtotumConfig.MACRO_RESPONSE
...
serial.close()
#write_status(False)
if os.path.isfile(FabtotumConfig.LOCK_FILE):
    os.remove(FabtotumConfig.LOCK_FILE)
#open(trace_file, 'w').close() #reset trace file
sys.exit()