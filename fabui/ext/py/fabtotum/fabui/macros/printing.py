...
def pause_additive(app, args=None, lang='en_US.UTF-8'):
    ...
    #app.macro("G91",                "ok", 2,    _("E relative position mode"), verbose=False )
    ...
    app.macro("G0 X210 Y210 Z{0} F6000".format(safe_z), "ok", 100,  _("Moving to Z safe zone"), verbose=False )
...
def resume_additive(app, args=None, lang='en_US.UTF-8'):
    ...
    if os.path.exists('/var/lib/fabui/settings/stored_task.json'):
        ...
        if "position" in content:
            ...
            app.macro("G0 E+60 F300",                   "ok", 300, _("Extrude filament to clean the nozzle and build up extrusion pressure") )
            app.macro("G92 E{0:.16f}".format(e),        "ok", 2,   _("Set extuder length"), verbose=False)
            app.macro("G90",                            "ok", 2,   _("Setting abs position"), verbose=False )
            app.macro("G0 X{0} Y{1} Z{2} F6000".format(x,y,z), "ok", 60,  _("Restore position"), verbose=False )
