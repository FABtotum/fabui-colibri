 ...
 */
	function checkURL() {
		...
		if (url) {
			...
			document.title = (title || page_title_prefix + document.title);
	...
	function loadURL(url, container) {
		...
		$.ajax({
			...
			beforeSend : function() {
				...
				// place cog
				container.html('<h1 class="ajax-loading-animation"><i class="fa fa-cog fa-spin"></i> '+_("Loading")+'</h1>');
			...
			error : function(xhr, status, thrownError, error) {
				pageLoadError(url, container,  xhr, thrownError);
			},
			async : true 
		});
 ...
 */ 
	function drawBreadCrumb(opt_breadCrumbs) {
		...
		bread_crumb.append($("<li>Home</li>")), a.each(function() {
			bread_crumb.append($("<li></li>").html($.trim($(this).clone().children(".badge").remove().end().text()))), --b || (document.title = page_title_prefix + bread_crumb.find("li:last-child").text())
		});
		...
		if (opt_breadCrumbs != undefined) {
			$.each(opt_breadCrumbs, function(index, value) {
				...
				document.title = page_title_prefix + bread_crumb.find("li:last-child").text();
			});
		}
	}
