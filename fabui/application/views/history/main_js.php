<script type="text/javascript">
	...
	$(function() {
		...
		oTable = $("#history").dataTable({
			...
			"sAjaxSource": '<?php echo site_url('history/getTableData')?>', 
		...
		function fnFormatDetails(oTable, nTr)
		{
			...
			if(type == 'print' || type == 'mill')
			{
				...
				var stats_button = '<a style="margin-left:5px;" class="btn btn-xs btn-default" href="#filemanager/file/'+id_file+'/stats"><i class="fa fa-area-chart"></i> Stats</a>';
		...
		{
			var params = {start_date: start_date, end_date:end_date, type:type, status: status};
			var url = '<?php echo site_url("history/getTableData") ?>?' + jQuery.param(params);
		...
		function ReloadStats()
		{
			$("#s2").css({ opacity: 0.3 });
			var params = {start_date: start_date, end_date:end_date, type:type, status: status};
			var url = '<?php echo site_url("history/getStatsData") ?>?' + jQuery.param(params);
