...
class Tasks extends CI_Model {
	...
	function add_task($data = array()){
		...
		$this->db->set('start_date', date('Y-m-d H:i:s'), false);
	...
	function update($id, $data = array()){
		...
		if(isset($data['finish_date']) && $data['finish_date'] == date('Y-m-d H:i:s')){
			$this->db->set('finish_date',  date('Y-m-d H:i:s'), false);
    ...
    function delete($id_task){
        ...
        $this->update($id_task, array('status' => 'deleted', 'finish_date' =>date('Y-m-d H:i:s')));
	...
	function get_make_tasks($filters = array()){
		...
		$this->db->select('sys_tasks.id as id, sys_tasks.user as user, sys_tasks.controller as controller, sys_tasks.type as type, sys_tasks.status as status, sys_tasks.id_object as id_object, 
						 ...
						 (julianday(sys_tasks.finish_date) - julianday(sys_tasks.start_date)) as duration', false)
	...
	function get_last_creations($type, $limit_end = 10, $limit_start = 0){
		...
		$this->db->select('st.id_file as id, st.id_object as id_object, st.finish_date as finish_date, sys_files.raw_name, sys_files.note as note, st.status, (julianday(st.finish_date) - julianday(st.start_date)) as duration, sys_files.attributes as attributes, sys_files.file_type, sys_objects.obj_name as object_name', false)
